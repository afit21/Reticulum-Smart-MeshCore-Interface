#!/usr/bin/env bash
#
# install-interface.sh -- install the released SmartMeshCoreInterface.py (the
# main branch on GitHub) into a Reticulum
...
#
# or download it and run ./install-interface.sh; --help lists the options.
# dev-tools/install-interface-dev.sh is the same script for the development branch.
#
set -euo pipefail

usage() {
  cat <<'USAGE'
install-interface.sh -- install SmartMeshCoreInterface.py from GitHub (main
branch) into a Reticulum install's interfaces/ directory.

Defaults to the main branch and ~/.reticulum. The downloaded file is checked
...
rnsd, installing it with pip if it is not.
  ...
  ./install-interface.sh                       # main -> ~/.reticulum
  ./install-interface.sh --branch development  # (or use dev-tools/install-interface-dev.sh)
  ./install-interface.sh --config-dir ~/.reticulum_test
  ./install-interface.sh --check               # report only, install nothing
  ./install-interface.sh --force               # reinstall even if unchanged
  ./install-interface.sh --skip-deps           # don't check/install the meshcore library

Straight from GitHub (options go after `bash -s --`):
  curl -fsSL https://raw.githubusercontent.com/afit21/Reticulum-Smart-MeshCore-Interface/main/install-interface.sh | bash
  curl -fsSL https://raw.githubusercontent.com/afit21/Reticulum-Smart-MeshCore-Interface/main/install-interface.sh | bash -s -- --check
USAGE
}
