...
| Meshcore Routing | Experimental | Route broadcast traffic over channels, and direct traffic over direct messages. The interface keeps a cache of which RNS addresses route to each MeshCore contact. If a low amount of peers are known, the interface sends broadcast traffic over direct message to avoid unnecessary floods. A destination that keeps answering with delivery proofs is never backed off: since 2026-09-20 a proof that arrives over the MeshCore channel (before a direct path exists) clears the unknown-destination backoff just as one arriving direct does (it still teaches no route, since channel senders are unauthenticated). |
...
| Adaptive ACK Timing | Working | Measures the real round trip time to each peer and shortens its own ACK waits to match, instead of always waiting out a fixed worst case. It never waits longer than it did before, only shorter. A missed ACK under a measured wait now doubles the next wait (still capped at the firmware's estimate) instead of discarding the measurement outright — a slow path stays measured, a dead first hop is handled by Dead Hop Detection. The hop-aware ceiling on any ACK wait is `direct_ack_timeout_base` (5 s) + `direct_ack_timeout_per_hop` (3 s) per hop since 2026-09-20 (was 8 + 4): over 2670 field ACKs the largest that ever arrived was 3.8 / 6.1 / 8.2 / 7.3 s at 0-3 hops, so 5 / 8 / 11 / 14 s cuts off none and stops holding the radio for a reply that is not coming. The random listen after a missed ACK is `direct_post_send_listen_min`/`_max` 0.2-1.0 s (was 0.3-3). |
| Fragment Reconciliation | Experimental | After sending a fragmented message once, the interface asks the peer which fragments it actually holds and re-sends only the missing ones, instead of blindly repeating everything unacknowledged. Since alpha 0.1.1: once the peer provably holds part of the packet, the remaining fragments get a larger retry budget (`direct_fragment_finish_attempts`), a send that still fails is resumed under the same packet id if RNS retries the identical bytes while the peer's reassembly bucket is alive (`direct_fragment_resume_enabled`), and the receive-side duplicate filter lets through the contexts RNS itself never dedups (Resource parts and requests, keepalives, cache requests). Reconcile questions and answers travel one priority tier above bulk data (a bidirectional transfer once held an answer for 30 s behind the answering node's own bursts), an unanswered question is re-asked rather than answered with a speculative re-burst, and the answer budget widens with this node's own queue depth. Since alpha 0.1.2 (evening build) the reconcile answer budget is capped (`direct_completion_check_timeout_max`, 15 s; 18 s from two hops) and each query carries a nonce (completion protocol v3, so **both nodes must run this build** -- an older peer ignores a v3 query). Since the 2026-09-19 night session: after its question is acknowledged the asking node keeps its radio quiet for a short hop-scaled window after the question's ACK (`direct_completion_quiet_base` 2 s + `direct_completion_quiet_per_hop` 3 s per hop, growing with the measured round trip and never longer than the answer budget) so its own next burst cannot collide with the answer at the repeater -- through a repeater the answering node is a hidden node, and that collision took one-hop answer delivery from 85% to 48% between two builds while zero hop stayed at 96-100%; the rest of the wait is still radio-free so a node's own answers never queue behind it. The per-peer in-flight cap (`direct_fragmented_max_in_flight`, 2) is priority-aware, gives announces a slot of their own, and never drops: a send that cannot get a slot in time goes anyway. Its one field session with the old dropping version went badly, but with the drop removed the simulated one-hop page transfer finishes in about three minutes with it and does not finish in ten without it, so it stays on pending a field check. Since 2026-09-20 the receiver of a raw burst **reports** what it holds without being asked (`direct_raw_report_enabled`, yes): the moment the packet completes, or -- when one of the burst's last two fragments (both carry a report flag) lands with gaps -- its bitmap; the sender keeps its radio quiet for `direct_raw_report_wait_base` (2 s) + `direct_raw_report_wait_per_hop` (3 s) per hop, re-drives from the second-last fragment's report if the last fragment's never comes, and only asks (the QUERY above) if no report arrives at all. The question and its ACK leave the common path, and the sender is silent exactly when the receiver transmits its own reply (a delivery proof, the next Resource request), which is what the two used to collide on at every hop count. A QUERY whose own firmware ACK is missed now waits only `direct_completion_unacked_grace` (6 s; `_multihop` 10 s from two hops) for its answer instead of the full budget -- such queries were answered 14% of the time in the field, never later than 6 s. |
