...
- **Queue hygiene.** Repeated path requests for the same destination are coalesced over a 20 s window, and any queued packet (except an announce) older than `outgoing_max_age` (120 s) is dropped rather than delivered as a stale burst once a path comes back.
