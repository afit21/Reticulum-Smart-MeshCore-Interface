...
- **Give up fast on a dead first hop.** On a multi-hop path the radio normally overhears the first repeater forwarding this node's own frame within a couple of seconds. Once a peer's echo timing has been measured, an attempt that has produced neither an ACK nor that echo by about twice the slowest echo seen (at least 5 s) is abandoned instead of waiting out the full firmware timeout (up to 28 s at three hops in real captures), so a stale path is reset in tens of seconds rather than minutes. `direct_hop1_abort_enabled = no` turns it off.
- **Queue hygiene.** Repeated path requests for the same destination are coalesced over a 20 s window; a packet whose exact bytes are already queued or being sent is dropped (RNS's Resource layer re-requests parts faster than a throttled radio can drain them); and any queued packet older than `outgoing_max_age` (120 s) is dropped before its first transmission rather than delivered as a stale burst once a path comes back. Announces and Resource data parts never expire, and nothing is ever dropped part-way through a fragmented send.
- **Duty-cycle accounting uses real airtime.** The transmit cap is 30 % of a rolling 60 s window (`duty_cycle_max_fraction` / `duty_cycle_window`; the window was 10 s until 2026-09-18, which quantized bursts to about 26 %), and the per-frame airtime it charges comes from the LoRa time-on-air formula at the radio's own SF/BW/CR (read at startup) and the firmware's real on-air framing, instead of a flat bitrate guess that was rounding a third fragment per window out of the budget. Link-maintenance traffic (link requests, proofs, keepalives, link close, Resource proofs and cancels) never waits for budget, so a page burst can't cost you the Link; its airtime is still charged to the window (`duty_cycle_exempt_handshake = no` to disable).
